import { BetterAuthError, DBFieldAttribute, DBFieldType, generateId } from "better-auth";
  ...
  createAdapterFactory({
    ...
    adapter: ({
      ...
      function convertWhereToFindOptions(
        ...
        action: "create" | "update" | "updateMany" | "findOne" | "findMany" | "delete" | "deleteMany" | "count",
          ...
          try {
            ...
            for (const modelName of expectedTables) {
              ...
              if (!tableExists) {
                ...
                const addColumns: { name: string; field: DBFieldAttribute<DBFieldType>; }[] = [];
